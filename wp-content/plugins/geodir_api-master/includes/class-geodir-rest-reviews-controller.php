 ...
 * Access comments
 */
class Geodir_REST_Reviews_Controller extends WP_REST_Comments_Controller {
	...
	public function get_items( $request ) {
		$prepared_args = array(
			...
			'orderby'         => $this->normalize_query_param( $request['orderby'], true, $request['order'] ),
			'order'           => $request['order'],
        ...
        if ( empty( $request['post_type'] ) ) {
            $prepared_args['post_type'] = geodir_get_posttypes();
        } else {
            $prepared_args['post_type'] = (array)$request['post_type'];
        }
	...
	public function prepare_item_for_response( $comment, $request ) {
        ...
        if ( isset( $comment->post_type ) ) {
            $ratings = maybe_unserialize( $comment->ratings );
            $comment_images = !empty( $comment->comment_images ) ? explode( ",", $comment->comment_images ) : array();
            
            $data['review_id'] = $comment->review_id;
            $data['post_title'] = $comment->post_title;
            $data['post_type'] = $comment->post_type;
            $data['rating_ip'] = $comment->rating_ip;
            $data['ratings'] = $ratings;
            $data['overall_rating'] = $comment->overall_rating;
            $data['comment_images'] = $comment_images;
            $data['wasthis_review'] = $comment->wasthis_review;
            $data['review_status'] = $comment->review_status;
            $data['review_post_status'] = $comment->review_post_status;
            $data['post_date'] = $comment->post_date;
            $data['city'] = $comment->city;
            $data['region'] = $comment->region;
            $data['country'] = $comment->country;
	 ...
	 */
	protected function normalize_query_param( $query_param, $ordering = false, $order = '' ) {
		$prefix = 'comment_';
		...
		switch ( $query_param ) {
			...
			case 'include':
				...
				break;
            case 'rating':
				$normalized = $order == 'desc' ? 'high_rating' : 'low_rating';
				break;
            case 'images':
				$normalized = $order == 'desc' ? 'highest_images' : 'least_images';
				break;
            case 'review':
				$normalized = $order == 'desc' ? 'high_review' : 'low_review';
				break;
            case 'include':
				$normalized = 'comment__in';
				break;
			default:
                if ( $ordering && in_array( $ordering, array_keys( geodir_rest_get_review_sorting() ) ) ) {
	...
	public function get_collection_params() {
        ...
        if ( function_exists( 'geodir_reviewrating_plugin_activated' ) ) {                           
            $orderby_rendered = geodir_rest_get_review_sorting();
            $orderby = array_keys( $orderby_rendered );
                        
            $orderby = !empty( $orderby ) ? array_unique( $orderby ) : array( 'date' );
            $orderby = apply_filters( 'geodir_rest_reviews_rating_comment_orderby', $orderby );
            ...
            $default_order = 'desc';
            $default_orderby = $comment_sorting;
            if ( !( $default_orderby && in_array( $default_orderby, array_keys( geodir_rest_get_review_sorting() ) ) ) ) {
                $default_orderby = 'latest';
            }
        } else {
            $default_order = 'desc';
            $default_orderby = 'date';
            $orderby = array( 'date' );
            $orderby_rendered = array();
        }
        
        $default_order = apply_filters( 'geodir_rest_reviews_rating_comment_default_order', $default_order );
        $default_orderby = apply_filters( 'geodir_rest_reviews_rating_comment_default_orderby', $default_orderby );
        ...
        $query_params['orderby_rendered']    = array(
			'description'           => __( 'All sorting options.' ),
			'type'                  => 'array',
	...
	protected function check_read_post_permission( $post ) {
		$posts_controller = new Geodir_REST_Listings_Controller( $post->post_type );
