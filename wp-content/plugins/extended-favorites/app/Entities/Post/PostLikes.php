...
class PostLikes
{
	...
	*/
	public function getUsers()
	{
		...
		foreach($users as $key => $user){
			if ( !$this->user_repo->isLike($this->post_id, $this->site_id, $user->ID) ){
				unset($users[$key]);
			}
		}
		return $users;
	...
	*/
	...
	{
		$total_count = $this->like_count->getCount($this->post_id, $this->site_id);
	...
	*/
	public function userList($separator, $include_anonymous, $anonymous_label, $anonymous_label_single)
	{
		...
		return apply_filters('likes_user_list', $out, $users, $anonymous_count);
	}
}