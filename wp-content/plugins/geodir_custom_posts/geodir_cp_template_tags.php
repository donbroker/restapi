...
function geodir_cp_listing() {
    ...
    jQuery(".delete_posttype").click(function() {
        ...
        var confirm_post = confirm('<?php _e('Are you wish to delete this Post Type?', 'geodir_custom_posts'); ?>');
        if (confirm_post) {
...
function geodir_cp_add_edit_form() {
    ...
    if ( isset ( $_REQUEST['geodir_save_post_type'] ) ) {
        ...
        $listing_slug = sanitize_text_field(stripslashes($_REQUEST['geodir_listing_slug']));
...
function geodir_cp_generate_post_type_order() {
    ...
    if ($all_postypes) {
        foreach($all_postypes as $postype) {
            $values[] = (int) $postype['listing_order'];
        }
    }
