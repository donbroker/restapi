...
function geodir_link_cpt_meta_box_add()
{
    ...
    if($link_business){
        ...
        if(isset($post->post_type) && isset($geodir_linked_post_types[$post->post_type])){
            $linked_post_type = $geodir_linked_post_types[$post->post_type];
        } else {
...
function geodir_link_cpt_business_fields_html() {
    ...
    if($link_business){
        ...
        if(isset($linkable_post_type) && isset($geodir_linked_post_types[$linkable_post_type])){
            $linked_post_type = $geodir_linked_post_types[$linkable_post_type];
        } else {
            $linked_post_type = 'gd_place';
        }
        ...
        if (isset($geodir_post_types[$linked_post_type])){
            $geodir_post_type = $geodir_post_types[$linked_post_type]['labels']['name'];
            $field_title = __( 'Link ', 'geodir_custom_posts' ).$geodir_post_types[$linked_post_type]['labels']['singular_name'];
        }
...
function geodir_detail_page_link_cpt_business_tab( $tabs_arr ) {
    ...
    if ( !empty($post) && !empty($post->ID) && !empty( $tabs_arr ) && $post_type != 'gd_event' && in_array( $post_type, $all_postypes ) && ( geodir_is_page( 'detail' ) || geodir_is_page( 'preview' ) ) ) {
        ...
        if ( !empty( $listing_ids ) ) {
            $html = geodir_cpt_link_businesses_data( $post_type, $listing_ids, $list_sort, $post_number );
            if ( $html ) {
                $geodir_linked_post_types = get_option('geodir_linked_post_types');
...
function geodir_cpt_link_businesses_data( $post_type, $post_ids, $list_sort = 'latest', $post_number = 5 ) {
    global $wpdb, $plugin_prefix;
