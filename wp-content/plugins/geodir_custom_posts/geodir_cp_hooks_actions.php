...
function geodir_single_next_previous_fix($url,$link,$direction,$post) {
	...
	if(isset($post->city_slug) && $post->city_slug != '') {
		...
		if($direction==__('Next', 'geodir_custom_posts')){$op = '>'; $order = "ASC";}else{$op = '<'; $order = "DESC";}
		$table = $plugin_prefix.$post->post_type.'_detail';

			$pid = $wpdb->get_var(
				$wpdb->prepare(
				"SELECT  post_id FROM ".$table." WHERE submit_time $op %d  AND post_status='publish' $where AND post_id !=%d ORDER BY submit_time $order  LIMIT 1",
                    $prep_arr
				)
			);
